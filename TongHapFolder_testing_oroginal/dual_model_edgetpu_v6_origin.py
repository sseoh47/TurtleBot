...
def parse_lane(outputs, H, W):
    ...
    기존 seg용 parse_lane()은 더 이상 사용하지 않음.
    """
...
def _raw_classify(shapes):
    ...
    # B) top eeu(or horizontal-like) + right line
    # 네 설명상 "커브처럼 보여도 실제론 eeu + lane으로 나올 상황"
    top_eeu = _has_class(shapes, "eeu", y="top")
    right_line_any = _has_class(shapes, "line", x="right")

    left_t_cond = left_bottom_curve or (top_eeu and right_line_any)

    # ---------- down_t ----------
    # 화면 하단에 eeu가 길게 위치
    # => bottom에서 eeu가 좌/중/우로 넓게 깔린 형태
    ...
    # 네 요구:
    # - straight는 최대한 유지
    # - 다만 하단 1/3에 들어온 left_t / down_t는 회전 우선
    #
    # 그래서:
    # 1) 하단 기반 left_t / down_t 먼저
    # 2) cross
    # 3) 그 외 None (straight는 convert 쪽에서 lane_status로 처리)
    #
    ...
    # mid/bottom eeu + right line 기반 left_t
...
def convert_lane_result(p_le, p_ls, p_is, p_it):
    ...
    return 1, float(round(p_le, 2))  ## 바뀜
