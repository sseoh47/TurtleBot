...
def compute_steer(mask: np.ndarray, last_steer: float) -> tuple[float, int, int]:
    ...
    elif left_detected and right_detected and len(left_points) >= 2 and len(right_points) >= 2:
        left_slope = fit_slope(left_points)
        right_slope = fit_slope(right_points)

        if left_slope is not None and right_slope is not None:
            return normalize_steer(left_slope + right_slope, SLOPE_SUM_LIMIT), left_count, right_count
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fast lane sender for MAIN_V2")
    ...
    camera = build_camera(args.width, args.height, args.fps)
    last_steer = 0.0
    last_debug_time = 0.0
